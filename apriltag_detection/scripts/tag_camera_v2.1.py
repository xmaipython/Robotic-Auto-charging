...
FINAL_BACK_DISTANCE = 0.1  # 最终后退距离(m)

# 控制参数
KP_X = 0.6  # X方向比例系数
KP_Y = 1.1  # Y方向比例系数
KP_Z = 3.1 # Z轴旋转比例系数
ANGLE_ERROR_THRESHOLD = 0.0046  # 角度误差阈值
POSITION_ERROR_THRESHOLD = 0.0030  # 位置误差阈值
...
class DockingController:
    ...
    def handle_fine_adjusting_state(self, tag0, tag1):
        ...
        # 状态检查
        angle_ok = abs(angle_error) < ANGLE_ERROR_THRESHOLD
        pos_ok = abs(y_error) < POSITION_ERROR_THRESHOLD
        
        cmd_vel = Twist()
        LIMITED_SPEED = 0.02  # 严格限速值
        ...
        if angle_ok and pos_ok or in_limited_mode:
            if not in_limited_mode:
                ...
                rospy.loginfo("进入锁定限速模式 (永久保持v_max=0.02m/s)")
            
            # 严格速度限制（完全忽略比例计算）
            if angle_error > 0:
                cmd_vel.angular.z = LIMITED_SPEED
            elif angle_error < 0:
                cmd_vel.angular.z = -LIMITED_SPEED
            else:
                cmd_vel.angular.z = 0
