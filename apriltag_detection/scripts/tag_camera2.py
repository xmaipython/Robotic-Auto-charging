...
# 让标签0处于-0.0580这个位置就是标签与相机对准
TARGET_DISTANCE_Y = -0.047  # 0、1标签中心坐标对齐点
TARGET_DISTANCE_Y_ERROR = 0.0048
# 状态定义
...
def tag_callback(msg):
    ...
    if 0 not in tag_positions and 1 not in tag_positions:
        ...
        y_diff = tag0_z - tag1_z
        rospy.loginfo(f"y_diff{y_diff:.3f}")
        
        if abs(y_diff) > ANGLE_ERROR_THRESHOLD:
            # 根据高度差调整角速度
            ...
            # 左右调整
            if center_x < TARGET_DISTANCE_Y - TARGET_DISTANCE_Y_ERROR:
                cmd_vel.linear.y = -LINEAR_SPEED
            elif center_x > TARGET_DISTANCE_Y + TARGET_DISTANCE_Y_ERROR:
                cmd_vel.linear.y = LINEAR_SPEED
            else:
