...
4. /odom_combined (geometry_msgs/PoseWithCovarianceStamped) - 里程计信息
...
MIN_LINEAR_SPEED_X = 0.12
MAX_ANGULAR_SPEED = 0.2
MIN_ANGULAR_SPEED = 0.06 

# 目标位置
TARGET_CENTER_Y = -0.039  # 充电桩中心Y坐标
TARGET_DISTANCE_Z = 0.45  # 目标停止距离
FINAL_BACK_DISTANCE = 0.08 / 10  # 最终后退距离

# 控制参数
KP_Y = 1.1  # Y方向比例系数
KP_Z = 0.6  # Z方向比例系数
ANGLE_ERROR_THRESHOLD = 0.005  # 角度误差阈值
POSITION_ERROR_THRESHOLD = 0.0032  # 位置误差阈值
...
class DockingController:
    def __init__(self):
        ...
        rospy.Subscriber('/odom_combined', PoseWithCovarianceStamped, self.odom_callback)
    ...
    def handle_approaching_state(self, tag0, tag1):
        ...
        cmd_vel = Twist()
        cmd_vel.linear.x = -max(min(abs(KP_Z*z_error), MAX_LINEAR_SPEED), MIN_LINEAR_SPEED_X)
    ...
    def handle_fine_adjusting_state(self, tag0, tag1):
        ...
        center_x = (tag0.position.x + tag1.position.x)/2
        y_diff = tag0.position.z - tag1.position.z
        cmd_vel = Twist()
        angle_ok = abs(y_diff) < ANGLE_ERROR_THRESHOLD
        pos_ok = abs(center_x - TARGET_CENTER_Y) < POSITION_ERROR_THRESHOLD

        rospy.loginfo(f"y_diff:{center_x-TARGET_CENTER_Y:.3f}||angle_diff:{y_diff:.3f}||angle_ok:{angle_ok}||pos_ok:{pos_ok}")

        if angle_ok and pos_ok:
            self.state = "FINAL_BACK"
            if self.odom_pose:
                self.back_start_x = self.odom_pose.x
            rospy.loginfo("开始最终后退")
        else:
            if not angle_ok:
                cmd_vel.angular.z = -MIN_ANGULAR_SPEED if y_diff > 0 else MIN_ANGULAR_SPEED
            if not pos_ok:
                cmd_vel.linear.y = -MIN_LINEAR_SPEED if center_x < TARGET_CENTER_Y else MIN_LINEAR_SPEED
    ...
    def handle_final_back(self):
        ...
        if distance_moved >= FINAL_BACK_DISTANCE:
            ...
            speed = -0.05 if (FINAL_BACK_DISTANCE - distance_moved) < 0.01 else -0.1
            rospy.loginfo(f"剩余距离:{10*(FINAL_BACK_DISTANCE-distance_moved):.3f}m")
            cmd_vel = Twist()
